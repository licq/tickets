...
prawn_document(:page_layout => :landscape) do |pdf|
  ...
  pdf.text "#{@agent.name}操作员业绩报表(#{@start_time.to_date}~#{@end_time.to_date})",:size => 30
  ...
  items = @table.map do |line|
    [line.user_name, line.count_sum, line.adult_ticket_sum.to_i, line.child_ticket_sum.to_i,line.adult_true_ticket_sum.to_i,line.child_true_ticket_sum.to_i, line.price_sum.to_i]
  end
  pdf.table items, :border_style => :grid,:width => 700,
            :headers => ["操作员","订单总数", "预订成人票", "预订儿童票", "实际成人票","实际儿童票","总价"],
